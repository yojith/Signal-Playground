...
@register_modify("band_stop")
def band_stop(signal: Signal, low_cutoff_hz: float, high_cutoff_hz: float, num_taps: int = 101) -> Signal:
    ...
    impulse = np.zeros_like(band_kernel)
    center = (len(band_kernel) - 1) // 2
    impulse[center] = 1.0
    stop_kernel = impulse - band_kernel

    filtered = np.convolve(signal.data, stop_kernel, mode="same")
